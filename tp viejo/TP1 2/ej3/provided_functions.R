...
#'
load_df <- function(filepath, dataset_name, var_to_predict, num_cols_to_remove = 0) {
  ...
  tree <- rpart(data_df[[var_to_predict]] ~ ., data = data_df, control = rpart.control(minsplit = 2, minbucket = 1, maxdepth = 10, cp = 0, xval = 0))
  variable_importance <- tree$variable.importance
  
  # Identify the least important variables
  less_important_vars <- names(sort(variable_importance, decreasing = FALSE)[1:num_cols_to_remove])
  
  # Remove the least important variables from the data frame
