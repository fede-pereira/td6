...
#'
#' @details
#' This function iterates through the given datasets, imputation methods, proportions
#' of missing data, and number of removed variables. For each combination, it configures
#' the preprocessing options, performs the experiment, and stores the results in a list.
#' The list of results is then combined into a single data frame, which is saved to the specified file.
#'
run_experiment <- function(datasets_to_pred, filepath) {
  ...
  # Iterate through different dataset, imputation, proportion of missing values, and num_vars_to_remove combinations
  for (dtp in datasets_to_pred) {
    for (impute in c("Yes")) {
      for (prop_NAs in c(0)) {
        for (num_vars_to_remove in 1:3) {  # Change this to the desired number of removed variables
          print(c(dtp$dataset_name, impute, prop_NAs, num_vars_to_remove))
          ...
          if (PARALLELIZE == TRUE) {
            res_tmp <- est_auc_across_depths(dtp, preprocess_control,
                                             ...
                                             val_reps = 30, num_vars_to_remove = num_vars_to_remove)
          } else {
          ...
          res_tmp$num_vars_to_remove <- num_vars_to_remove
...
#' for different datasets, proportions of missing data, and number of removed variables. The resulting plot
#' is saved as the specified file.
#'
plot_exp_results <- function(filename_exp_results, filename_plot, width, height) {
  ...
  data_for_plot <- exp_results %>%
    group_by(dataset_name, prop_NAs, IMPUTED, maxdepth, num_vars_to_remove) %>%
  ...
  g <- ggplot(data_for_plot, aes(x = maxdepth, y = mean_auc, color = IMPUTED)) +
    ...
    facet_grid(dataset_name + prop_NAs ~ num_vars_to_remove, scales = "free_y") +
...
datasets_to_pred <- list(
  load_df("/Users/ezequielkaplan/Downloads/TP1\ 2/src/data/customer_churn.csv", "Churn", "churn"), # Source: https://archive.ics.uci.edu/dataset/563/iranian+churn+dataset
  load_df("/Users/ezequielkaplan/Downloads/TP1\ 2/src/data/heart.csv", "Heart", "HeartDisease"),    # Source: https://www.kaggle.com/datasets/arnabchaki/data-science-salaries-2023
