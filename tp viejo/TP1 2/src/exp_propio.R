...
source("/Users/ezequielkaplan/Downloads/TP1 2/ej3/provided_functions_exp_propio.R")
...
run_experiment <- function(datasets_to_pred, filepath) {
  ...
  for (dtp in datasets_to_pred) {
    for (impute in c("No")) {
      for (elimino in c(0,1,4,5)) {

        dtp_modified <- eliminar_columnas_menos_importantes(dtp, elimino)
